...
public class OrdersIntegrationTests
{
    ...
    {
        using var stream = File.OpenRead(@"Orders\sample_order_opentrans_2_1_xml signature.xml");

        var content = new StreamContent(stream);
        content.Headers.ContentType = new MediaTypeHeaderValue("application/xml");

        var result = await client.PostAsync("OrderValidation/via-stream", content);
    ...
    {
        using var stream = File.OpenRead(@"Orders\sample_order_opentrans_2_1_xml signature - invalid.xml");
    ...
    public async Task Can_parse_valid_Invoice_via_model_binding()
    {
        ...
        var result = await client.PostAsync("OrderValidation/via-model-binding", content);
    ...
    public async Task Can_parse_valid_Invoice_via_file()
    {
        ...
        {
            {new StreamContent(stream), "file", @"Orders\sample_order_opentrans_2_1_xml signature.xml"}
        };

        var result = await client.PostAsync("OrderValidation/via-file", content);
        var r = await result.Content.ReadAsStringAsync();
        result.StatusCode.Should().Be(HttpStatusCode.OK);
    }
    ...
    public async Task Cannot_parse_invalid_Invoice_via_file()
    {
        ...
        var content = new MultipartFormDataContent
        {
            {new StreamContent(stream), "file", @"Orders\sample_order_opentrans_2_1_xml signature - invalid.xml"}
        };
